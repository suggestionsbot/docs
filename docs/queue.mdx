...
## Usage
To enable the suggestions queue, run `/configure guild` and configure the queue related items.  From here, you may manage the queue in one of two ways:
...
In order to configure the physical queue, run the following command:
- `/configure guild menu:Configure Channel Based Suggestions Queue and Logs`

This will have suggestions being sent to a channel in your discord for review.
...
When you hit the red 'x' button, you reject and remove the suggestion from the queue. When this happens, the bot will also attempt to [DM the author](guild-configuration#dm-responses) of the suggestion about their suggestion's rejection.
