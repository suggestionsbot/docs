...
To start allowing the submission of anonymous suggestions, enable the feature with the `/configur eguild` command. The feature is enabled by default.
...
By enabling anonymous resolutions, you protect your staff member's identities when resolving suggestions and adding notes.

With it enabled, the process runs similarly to everything above. In this case, the moderator who resolved the suggestion can mark themselves as anonymous. Not only does this apply to the suggestion message itself, but the DM sent to the author as well.
...
While the actual submitter of the suggestion remains anonymous, the voters are not anonymous. Currently, the feature is mainly designed for the suggestion submitter itself.
