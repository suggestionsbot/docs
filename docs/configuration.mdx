...
Users are also able to configure their own DM settings using the `/dm <enable|disable>` command. This allows a user to choose their own preferences at the guild level. However, they cannot override your server's settings.
