...
import SuggestionEmbedMessage from '@site/src/components/SuggestionEmbedMessage';
import SuggestionThreadMessage from '@site/src/components/SuggestionThreadMessage';
...
## Suggestions Channel
Usage: `/config get config:Suggestions channel` (to view) or `/config channel:<channel>` (to set)
...
## DM Responses
Usage: `/config get config:Dm responses` or `/config dm <enable|disable>`
...
Usage: `/config get config:Threads for suggestions` or `/config thread <enable|disable>`
...
Usage: `/config get config:Keep logs` or `/config keeplogs <enable/disable>`
...
[//]: # (TODO: Post GIFs of the approval/rejection with both keeplogs enabled and disabled)
